...
export function ImagePicker({ images, selected, disabled, onSelect }: { images: string[]; selected: string; disabled?: boolean; onSelect: (url: string) => void }) {
  ...
  return (
    <div className="rounded-[8px] border border-[var(--line)] bg-black/20 p-3">
      ...
      <button type="button" disabled={disabled || !active} onClick={() => active && onSelect(active)} className={`group relative h-56 w-full overflow-hidden rounded-[8px] border ${selected === active ? 'border-[var(--accent)]' : 'border-[var(--line)]'}`}>
